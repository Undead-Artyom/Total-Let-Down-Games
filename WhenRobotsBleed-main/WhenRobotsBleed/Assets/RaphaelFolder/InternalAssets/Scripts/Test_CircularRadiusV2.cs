...
public class Test_CircularRadiusV2 : MonoBehaviour
{
    ...
    void OnDrawGizmos(){
        if(_closestGrapplePointLocation != Vector3.zero){
            Debug.DrawRay(this.transform.position, _closestGrapplePointLocation-this.transform.position, _closestGrapplePointColor, 0.01f, false);
            /*
    ...
    private void UpdateClosestGrapplePoint(
        ...
        if(dictionaryOfGrapplePoints.Count == 0){
            ...
            foreach(KeyValuePair<string, GameObject> kV in dictionaryOfGrapplePoints){
                ...
                /*
                if()
                _closestGrapplePointSpriteRenderer = kV.Value.gameObject.GetComponent<SpriteRenderer>()
                _closestGrapplePointSpriteRenderer.color = _closestGrappleDistanceColor;
                */
